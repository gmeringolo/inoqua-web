...
export function ServiceCard({ icon, title, text, href }: ServiceCardProps) {
  return (
    <Link
      href={href}
      className="group flex h-75 w-full flex-col rounded-card bg-azul p-7 transition-transform duration-300 hover:-translate-y-2 hover:shadow-xl"
    >
      {/* Acento de color en vez del ícono de Figma — reemplazá por tu ícono real si querés */}
      <Icon name={icon} className="h-8 w-8 text-naranja" />
      <h3 className="mt-6 min-h-13 text-card-titulo text-white">{title}</h3>
      <p className="mt-4 text-card-txt text-white/90">{text}</p>
      <span className="mt-auto flex items-center gap-2 text-sm text-white transition-transform duration-200 group-hover:translate-x-1">
        Ver más
